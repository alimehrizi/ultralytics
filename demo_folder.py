...
folder_path = '/mnt/DD5/PedestrainDetection/CUHK-SYSU/images'
save_path = folder_path+'_predictions_2'
import os 
...
for n,img_path in enumerate(glob.glob(folder_path+'/*.jpg')):
    ...
    outputs = non_max_suppression(outputs.cpu(),0.4)[0]

    scale_coords(img2.shape[1:], outputs[:, :4], shapes[0], shapes[1])  # native-space pred
    false_classes = [] 
    ...
    for idx in range(outputs.size(0)):
        xywh, conf, cls, embed = outputs[idx,:4].numpy(),outputs[idx,4].item(),outputs[idx,5].item(),outputs[idx,6:].numpy()
        if conf>=0.5:
        ...
        # img2 = cv2.rectangle(img, (x-w//2, y-h//2),(x+w//2, y+h//2),(0,255,0),2)
        # cv2.imwrite(save_path + '/'+image_name,img2)
        # cv2.putText(img_back,lut[int(cls)],(boxj[0], boxj[1]),4,1,(255,0,0),2)
    json.dump([annot],f)
    f.close()
